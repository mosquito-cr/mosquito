...
describe "Overseer autoscaling integration" do
  ...
  describe "scale down" do
    it "releases idle executors when autoscaler recommends fewer" do
      clean_slate do
        ...
        assert overseer.released_executors.first.decommissioned?
      end
    end
  ...
  describe "executor decommissioned flag" do
    it "defaults to false" do
      executor = overseer.executors.first
      refute executor.decommissioned?
    end

    it "can be set via decommission!" do
      executor = overseer.executors.first
      executor.decommission!
      assert executor.decommissioned?
    end
  end

  describe "adjust_executor_pool respects executor_count" do
    it "respawns executors up to executor_count" do
      clean_slate do
        # Set a low target
        overseer.executor_count = 1

        # Kill all current executors
        overseer.executors.clear

        # Run the pool adjustment
        overseer.adjust_executor_pool

        # Should only respawn up to executor_count (1), not config default
        assert_equal 1, overseer.executors.size
      end
    end
  end
